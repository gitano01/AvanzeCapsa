...
function validarFormularioProveedor() {
    if (!document.getElementById('btnGuardar').classList.contains("disabled")){
        ...
        var message, x;
        var band = 0;
        var campos = ["nombreP", "servicio", "contacto", "rfc","email1"];
        var msgs = ["msg_nombreP", "msg_servicio", "msg_contacto", "rfcmensaje","msemail"];
        var primerCampo;
        // RFC
        ...
        for (i = 0; i < 5; i++) {
            try {
                ...
                if (campos[i] == "nombreP" || campos[i] == "servicio" || campos[i] == "contacto") {
